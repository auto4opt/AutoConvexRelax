...
RESUME      = _env_bool("APA_RESUME", False)                     # 关掉续训就设 False
TRAIN_SEED  = _env_int("APA_TRAIN_SEED", 42)
CKPT_PATH   = _env_str("APA_CKPT_PATH", "./train_logs/checkpoints/checkpoint_400.pt")     # 手动指定 / 或者自动搜
...
LOG_DIR       = _env_str("APA_LOG_DIR", "outputs/logs/train/stage1")
CKPT_SAVE_DIR = _env_str("APA_CKPT_SAVE_DIR", os.path.join(LOG_DIR, "checkpoints"))
os.makedirs(CKPT_SAVE_DIR, exist_ok=True)
FINETUNE_FILE = _env_str("APA_FINETUNE_FILE", "outputs/data/vector_all_mix_1600.pkl")
...
DATASET_TAG = _env_str("APA_DATASET_TAG", "HYBRID_MIX_1600")
...
# NOTE: This variant disables solver-based lb improvement reward.
SOLVER_REWARD_ENABLE = False
SOLVER_REWARD_PROB   = _env_float("APA_SOLVER_REWARD_PROB", 1.0)      # 0.2: 只在 20% episode done 时算一次
SOLVER_REWARD_LAMBDA = 2.0      # solver reward 权重
SOLVER_REWARD_SCALE  = 0.25     # 25% 提升 -> tanh(1) 左右
...
class QCQPEnv:
    ...
    def _solver_reward(self, relaxed_prob: QCQPProblem):
        ...
        rl_lb = solve_convex_relax_mosek(p, time_limit=MOSEK_TL, verbose=False)  # returns float|None :contentReference[oaicite:9]{index=9}
        if rl_lb is None:
        ...
        shaped = math.tanh(pct / SOLVER_REWARD_SCALE)
        r_add = SOLVER_REWARD_LAMBDA * shaped
...
def main():
    ...
    # No root-lb cache used in this variant
    root_cache = {}
    ...
    try:
        ...
        # 2. (原 GROUP_UPDATES)
        # 默认单阶段训练 200 个 updates，可由 APA_GROUP_UPDATES 覆盖
        GROUP_UPDATES[:] = _env_int_list("APA_GROUP_UPDATES", [200])
        TOTAL_UPDATES = sum(GROUP_UPDATES)
        cum_updates[:] = np.cumsum(GROUP_UPDATES).tolist()
    ...
    if RESUME:
        # 续训: 尝试加载 'latest' checkpoint
        resume_path = os.path.join(CKPT_SAVE_DIR, "latest.pt")
        if os.path.isfile(resume_path):
            print(f"[Resume] 续训模式开启。加载 'latest' checkpoint: {resume_path}")
    ...
    for update in range(start_update, TOTAL_UPDATES + 1):
        ...
        # No root-lb cache filtering in this variant
        valid_idx_pool = list(train_indices)
        ...
        # --- 新增：每 50 轮保存一次 checkpoint ---
        if update % 10 == 0 or update == TOTAL_UPDATES: # 在第50轮和最后一轮保存
